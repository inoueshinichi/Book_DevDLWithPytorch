...
def NonMaximum_Suppression(boxes, scores, overlap=0.45, top_k=200):
    ...
    while idx_top_k.numel() > 0:
        ...
        torch.index_select(x1, dim=0, index=idx_top_k, out=tmp_x1)
        torch.index_select(y1, dim=0, index=idx_top_k, out=tmp_y1)
        torch.index_select(x2, dim=0, index=idx_top_k, out=tmp_x2)
        torch.index_select(y2, dim=0, index=idx_top_k, out=tmp_y2)
